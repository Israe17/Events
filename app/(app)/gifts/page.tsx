...
export default async function GlobalGiftsPage() {
  ...
  return (
    <div className="px-4 pt-8 space-y-6">
      <PageHeader title="Regalos" subtitle="Lista de regalos por evento" />

      {events.length === 0 ? (
        <EmptyState icon={Gift} title="Sin eventos" description="Únete a un evento para ver regalos" />
      ) : (
        <ul className="space-y-2.5">
          {events.map((ev: any) => (
            <li key={ev.id}>
              <Link href={`/events/${ev.id}/gifts`} className="card card-hover group flex items-center gap-4 p-4">
